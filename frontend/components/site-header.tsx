...
export function SiteHeader() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-16 border-b border-slate-200/80 bg-white/95 backdrop-blur-md z-40 flex items-center justify-between px-6 shadow-xs shadow-slate-100">
      <Link href={urls.home()} className="flex items-center gap-2.5 group">
        ...
        <span className="font-extrabold text-[19px] tracking-tight text-slate-900 group-hover:text-brand-600 transition-colors">
      ...
      <div className="flex items-center flex-1 max-w-md mx-8">
        <div className="relative w-full" ref={wrapRef}>
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-lg py-2 pl-9 pr-14 text-sm text-slate-600 placeholder-slate-400 focus:outline-hidden focus:ring-2 focus:ring-brand-500/30 focus:border-brand-400 transition-colors"
          />
          <kbd className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[11px] mono text-slate-400 bg-white border border-slate-200 rounded-sm px-1.5 py-0.5 pointer-events-none">
            ⌘K
          </kbd>
      ...
      {user && (
        <Link href={urls.userProfile(user.username)} className="flex items-center shrink-0">
          {user.profile_picture ? (
            // eslint-disable-next-line @next/next/no-img-element -- matches the original header markup.
            <img
