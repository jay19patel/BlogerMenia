...
# Load the backend .env file into os.environ
...
MILVUS_URI = str(BASE_DIR / "search" / "milvus.db")
...
# All CELERY_* settings are picked up by the Celery app in config/celery.py.
...
# In local dev (DEBUG=True), Milvus Lite locks the local milvus.db file, which
# prevents Celery and Django from running concurrently. We run tasks eagerly
# by default in local dev so only the Django process handles them.
